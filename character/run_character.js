const fs = require("fs");
const { createCharacter, allocateInitialSkillPoints } = require("./character_rank");
...
rl.question("What is this adventurer's name?: ", (name) => {
  ...
  const waitForDb = setInterval(() => {
    if (fs.existsSync(dbName)) {
      ...
      // ✅ Import primaryStats AFTER database exists
      const { primaryStats } = require("./character_rank");
...
function getSkillOptionsFromDb(tableName, callback) {
  const { getLatestDbFile } = require("./character_rank"); // ✅ Fixed the path
